...
static void cgen_driver(tree_t driver, cgen_ctx_t *ctx)
{
   tree_t decl = tree_ref(driver);

   LLVMValueRef nets = NULL;
   if (tree_has_value(driver))
      //   nets = cgen_signal_lvalue(tree_value(driver), ctx);
      assert(false);
   else
      nets = cgen_signal_nets(decl);
